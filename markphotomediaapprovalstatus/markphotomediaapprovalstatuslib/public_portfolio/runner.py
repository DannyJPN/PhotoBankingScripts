...
def _crawl_portfolio(adapter, context, portfolio_url: str, contributor_id: str) -> List[PublicAsset]:
    ...
    :return: List of PublicAsset objects.
    """
...
def process_public_portfolio_approval(
    ...
    for bank, adapter_cls in BANK_ADAPTERS.items():
        # Create separate context per bank to load bank-specific cookies
        with browser_context(headless=headless, bank=bank) as context:
            ...
            assets = _crawl_portfolio(adapter, context, portfolio_url, contributor_id)
