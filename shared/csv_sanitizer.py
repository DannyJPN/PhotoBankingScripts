...
class CSVSanitizer:
    """
    Sanitizes CSV data to prevent injection attacks.

    This class provides methods to clean CSV field values by:
    1. Neutralizing dangerous prefixes (=, +, -, @, tab, newlines)
    2. Escaping internal quotes properly
    3. Detecting and neutralizing suspicious patterns (cmd|, formulas, URIs)
    4. Quoting fields with special characters
    """
    ...
    SUSPICIOUS_PATTERNS = [
        r'cmd\|',  # Command pipe
        r'=.*\(',  # Formula with function (catches =HYPERLINK formulas)
        r'\+.*\(',  # Plus formula with function
        r'-.*\(',  # Minus formula with function
        r'@.*\(',  # @ formula with function
        r'\\\\.*\\',  # UNC paths (\\server\share)
        r'file:///',  # File URIs
        # NOTE: r'https?://' removed - legitimate URLs in descriptions are safe
        # The =.*\( pattern already catches =HYPERLINK(...) formula attacks
    ]
    ...
    def sanitize_field(value: Any) -> str:
        ...
        Examples:
            >>> CSVSanitizer.sanitize_field("=cmd|'/c calc'")
            "'=cmd|'/c calc'"
            >>> CSVSanitizer.sanitize_field("Normal text")
            "Normal text"
            >>> CSVSanitizer.sanitize_field("+SUM(1+1)")
            "'+SUM(1+1)"
            >>> CSVSanitizer.sanitize_field("Photo at https://example.com")
            "Photo at https://example.com"
        """
        ...
        for prefix in CSVSanitizer.DANGEROUS_PREFIXES:
        ...
        if not needs_neutralization:
            for pattern in CSVSanitizer.SUSPICIOUS_PATTERNS:
    ...
    def sanitize_record(record: Dict[str, Any]) -> Dict[str, str]:
        ...
        Examples:
            ...
            >>> CSVSanitizer.sanitize_record(record)
            {"name": "'=cmd|calc", "description": "Normal"}
        """
        sanitized = {}
        for key, value in record.items():
            sanitized[key] = CSVSanitizer.sanitize_field(value)
        return sanitized
    ...
    def sanitize_records(records: List[Dict[str, Any]]) -> List[Dict[str, str]]:
        ...
        Examples:
            ...
            >>> CSVSanitizer.sanitize_records(records)
            [{"title": "'=SUM(1+1)"}, {"title": "Normal"}]
        """
        return [CSVSanitizer.sanitize_record(record) for record in records]
    ...
    def is_dangerous(value: Any) -> bool:
        ...
        Examples:
            >>> CSVSanitizer.is_dangerous("=cmd|calc")
            True
            >>> CSVSanitizer.is_dangerous("Normal text")
            False
        """
        if not isinstance(value, str):
            value = str(value) if value is not None else ''
