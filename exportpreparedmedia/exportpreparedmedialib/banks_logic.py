...
def get_output_paths(enabled_banks: list[str], base_prefix: str) -> dict[str, str]:
    ...
    for bank in enabled_banks:
        ...
        logging.debug(
            f"Output directory exists: {os.path.exists(os.path.dirname(output_file) if os.path.dirname(output_file) else '.')}"
        )
...
def should_include_item(item: dict[str, str], bank: str = None) -> bool:
    ...
    """
    filename = item.get(
        "Soubor", item.get("filename", item.get("Filename", item.get("file", item.get("File", "unknown"))))
    )
    ...
    if bank:
        ...
        if bank_status_column:
            ...
            logging.debug(
                f"Checking if item {filename} should be included for {bank}: status='{status_value}', valid={is_valid}"
            )
            return is_valid
        else:
    ...
    for key in status_columns:
        ...
        if status_value == VALID_STATUS.lower():
            logging.debug(
                f"Item {filename} included: status '{status_value}' in column '{key}' matches valid status '{VALID_STATUS.lower()}'"
            )
            return True
...
def extract_media_properties(
    item: dict[str, str], category_maps: dict[str, dict[str, str]], pond_prices: dict[str, str] = None
) -> dict[str, str]:
    """
