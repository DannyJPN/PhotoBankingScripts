...
from exportpreparedmedialib.banks_logic import get_enabled_banks, get_output_paths, should_include_item
from exportpreparedmedialib.constants import DEFAULT_OUTPUT_FOLDER, DEFAULT_PHOTO_CSV, VALID_STATUS
...
def parse_arguments():
    parser = argparse.ArgumentParser(description="Export prepared media to various stock photo banks.")
    # Základní parametry
    parser.add_argument(
        "--photo_csv", type=str, default=DEFAULT_PHOTO_CSV, help="Path to the input CSV file with photo metadata"
    )
    parser.add_argument(
        "--output_folder", type=str, default=DEFAULT_OUTPUT_FOLDER, help="Path to the output folder for CSV files"
    )
    parser.add_argument("--debug", action="store_true", help="Enable debug logging")
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing output files")

    # Přepínače pro jednotlivé banky
    parser.add_argument("--shutterstock", action="store_true", help="Export to ShutterStock")
    parser.add_argument("--adobestock", action="store_true", help="Export to Adobe Stock")
    parser.add_argument("--dreamstime", action="store_true", help="Export to DreamsTime")
    parser.add_argument("--depositphotos", action="store_true", help="Export to DepositPhotos")
    parser.add_argument("--bigstockphoto", action="store_true", help="Export to BigStockPhoto")
    parser.add_argument("--123rf", dest="_123rf", action="store_true", help="Export to 123RF")
    parser.add_argument("--canstockphoto", action="store_true", help="Export to CanStockPhoto")
    parser.add_argument("--pond5", action="store_true", help="Export to Pond5")
    parser.add_argument("--gettyimages", action="store_true", help="Export to GettyImages")
    parser.add_argument("--alamy", action="store_true", help="Export to Alamy")
    parser.add_argument("--all", action="store_true", help="Export to all supported banks")
